...
try {
    ...
    function getFormData(e) {
        ...
        const textSizeRadio = Array.from(document.getElementsByName("text-size"));
        ...
        const textSize = (_b = textSizeRadio.find((item) => item.checked)) === null || _b === void 0 ? void 0 : _b.value;
        if (textSize != undefined)
            trainer.settings.textSize = textSize;
        else
            throw new Error("Text size is undefined");
    ...
    async function createText(language, textSize) {
        ...
        if (language === "Rus")
            url = `https://fish-text.ru/get?number=${sentenceCount}`;
        else
            url = `https://baconipsum.com/api/?type=meat-and-filler&sentences=${sentenceCount * 2}`;
